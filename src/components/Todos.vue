<template>
  <section class="todoapp">
    ...
    <header class="header">
      <h1>Tasks</h1>
      <input class="new-todo" autofocus autocomplete="off" v-model="newTodo" @keyup.enter="addNewTodo()" placeholder="What needs to be done?" />
    </header>
    ...
    <section class="main" v-show="todos.length" v-cloak>
      ...
      <ul class="todo-list">
        <li v-for="todo in filteredTodos" :key="todo.id" class="todo" :class="{completed: todo.completed, editing: todo == editingTodo}">
          ...
          <input class="edit" type="text" v-model="todo.title" v-focus="todo == editingTodo" @blur="doneEditTodo(todo)" @keyup.enter="doneEditTodo(todo)" @keyup.esc="cancelEditTodo(todo)">
        </li>
      </ul>
    </section>

    <footer class="footer" v-show="todos.length" v-cloak>
      <span class="todo-count" v-show = "remainingTodos">
        <strong>{{ remainingTodos }}</strong> {{ remainingTodos | pluralize }} left
      </span>
      <ul class="filters">
        <li>
          <a href="#" @click="setFilterTodos('all')">All</a>
        </li>
        <li>
          <a href="#" @click="setFilterTodos('active')">Active</a>
        </li>
        <li>
          <a href="#" @click="setFilterTodos('completed')">Completed</a>
        </li>
        <li>
          <a href="#" @click="removeCompleted()">Clear completed</a>
        </li>
      </ul>
    </footer>
